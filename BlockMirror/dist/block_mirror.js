...
BlockMirrorBlockEditor.prototype.toolboxPythonToBlocks = function (toolboxPython) {
  ...
  return toolboxPython.map(function (category) {
    ...
    console.log("BlockMirrorBlockEditor.prototype.toolboxPythonToBlocks: ", category.name, category.colour);
...
BlockMirrorBlockEditor.prototype.makeToolbox = function () {
  ...
  for (var name in BlockMirrorBlockEditor.EXTRA_TOOLS) {
    console.log("BlockMirrorBlockEditor.prototype.makeToolbox: ", name);
...
BlockMirrorBlockEditor.prototype.TOOLBOXES = {
  ...
  }, TOOLBOX_CATEGORY.SEP, '<category name="表达式" expanded="true">', {
    name: "Constants",
    colour: "TEXT",
...
Blockly.Blocks['ast_List'] = {
  ...
  updateShape_: function updateShape_() {
    if (this.itemCount_ && this.getInput('EMPTY')) {
      ...
      this.appendDummyInput('EMPTY').appendField('建立空列表 []');
    } // Add new inputs.
  ...
  updateShape_: function updateShape_() {
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput('ADD' + i)) {
        ...
        if (i === 0) {
          input.appendField('建立字典').setAlign(Blockly.ALIGN_RIGHT);
        }
      }
    } // Remove deleted inputs.
...
Blockly.Blocks['ast_FunctionDef'] = {
  ...
  updateShape_: function updateShape_() {
    ...
    var block = this;
    var position = 1;
    [['DECORATOR', 'decoratorsCount_', null, 'decorated by'], ['PARAMETER', 'parametersCount_', 'Parameter', '参数:']].forEach(function (childTypeTuple) {
